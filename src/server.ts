...
import { MCPRequest, NotificationMessage } from "./types.js";
...
export class MCPServer {
  ...
  constructor(apiKey: string, port: number = 3005) {
    ...
    this.wss = new (WebSocket as any).Server({ server: this.httpServer });
  ...
  private handleError(ws: WebSocket, error: any): void {
    ...
    if (error instanceof SyntaxError) {
      ...
      error.code &&
      ERROR_CODES[error.code as keyof typeof ERROR_CODES]
    ) {
      this.sendError(
        ws,
        null,
  ...
  private monitorConnections(): void {
    ...
    this.clients.forEach((state, ws) => {
      ...
      const timeSinceLastMessage =
        (now.getTime() - state.lastMessageAt.getTime()) / 1000;
  ...
  private logError(type: string, error: Error, state?: any): void {
    const errorLog = {
